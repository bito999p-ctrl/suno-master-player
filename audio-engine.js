...
export function analyzeAudioResonances(buffer, userPresetKey) {
  ...
  if (rumbleNoiseFloorDb > -58.0) { // -55dB から -58dB へ引き下げ、微小な超低域ハムノイズも確実にカット
    sugRumbleCut = true;
  }

  let sugHissAmount = 0;
  if (hissNoiseFloorDb > -72.0) { // -62dB から -72dB へ引き下げ、ヘッドホン等で聞こえる微細なヒスノイズも高感度で検出
    // -72dB で 0%、-40dB で最大 90% になるよう感度比率を調整（2.8倍スケール）
    const rawHiss = Math.round(Math.max(0, Math.min(90, (hissNoiseFloorDb + 72.0) * 2.8)));
